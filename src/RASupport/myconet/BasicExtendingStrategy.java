  ...
  public void doDynamics(MycoNode node, HyphaData data, HyphaLink link) {
    ...
    if (link.getStable().size() < Math.floor(data.getParentTarget())) {
      ...
      if (o != null) {
		...
		/* RASUPPORT */ 
                ...
                // BUG: sometimes re-advertise to the same SP, i.e., sometimes selects as stable the same SP where is already connected 
		// Advertisement agent (initial) when a SP (node) transforms into NP and it is not able to connect to a stable hypha
		// The old SP searchs for a stable hypha
		if(node.isBiomass() && !node.contains(o)) {		
                      
                    // UNCOMMENT TRICK
                    //logMessage("SUPER-PEER " + o.getAlias() + " has received a RS from " + node.getAlias()); // TRICK
                    node.getRASupport().advertiseResourcesTo(o);
                    logMessage("BE1");
		}		
		/* END RASUPPORT */ 
    ...
    if (link.isOverBiomass()) {
      ...
      while (hyp.size() > Math.floor(data.getParentTarget())) {
        ...
        /* RASUPPORT */
        // UNCOMMENT TRICK
        // Sends a message to the super-peer candidate selected (candidate), in order to delete the entry (node) from the super-peer selected        
        if(node.isBiomass()) {
            //logMessage("SUPER-PEER " + candidate.getAlias() + " DELETES " + node.getAlias() + " from its database"); // TRICK
            candidate.getRASupport().deleteIndexEntry(node.getAlias());
        }        
        /* END RASUPPORT */
