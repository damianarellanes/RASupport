...
public class HyphaLink implements Linkable, CDProtocol, Cleanable {
  ...
  public void transferBiomass(MycoNode target, int quantity) {
    ...
    for (MycoNode bio : getBiomass()) {
      ...
      /* SUPPORT */
      //logMessage(bio.getID() + " Sends initial advertisement agent to " + target.getID() );      
      /* END SUPPORT */
    }
  ...
  public void transferNeighbor(MycoNode neighbor, MycoNode target) {
    if (neighbor != null && target != null) {
      ...
      //TO PROBE
      if(target.isBiomass() && (myNode.isExtending() || myNode.isBranching() || myNode.isImmobile())) {
          target.getRASupport().advertiseResourcesTo(myNode);
      }
      
        /* SUPPORT */     
        // UNCOMMENT
        // Advertisement agent (initial) when a SP transfers its neighbor (neighbor) to another SP (target)        
        if(neighbor.isBiomass()) {
              
            // Remove neighbor from the database            
            //logMessage("SUPER-PEER " + myNode.getAlias() + " DELETES " + neighbor.getAlias() + " from its database"); // TRICK
            myNode.getRASupport().deleteIndexEntry(neighbor.getAlias());
                        
            // Neighbor sends an initial advertisement agent to target
            //logMessage("SUPER-PEER " + target.getAlias() + " has received a RS from " + neighbor.getAlias()); // TRICK
            neighbor.getRASupport().advertiseResourcesTo(target);
            logMessage("HL1");
            
        }	
        // myNode trasnfer target (normal peer) to a extending/branching/immobile neighbor
        else if(neighbor.isExtending() || neighbor.isBranching() || neighbor.isImmobile()) {
            
            //myNode.getRASupport().deleteIndexEntry(target.getAlias());
            
            /*target.getRASupport().advertiseResourcesTo(neighbor);
            logMessage("HL3");*/
        }
      /* END SUPPORT */
    } else {
  ...
  public void absorbHypha(MycoNode target) {
    ...
    /* SUPPORT */      
    //if(oldState == EXTENDING && T == BIOMASS)
     // Advertisement agent (initial) when a SP (n) transforms into NP         
	//SupportResourceAggregation.sendAdvertisementAgent(target, myNode, ADVERTISEMENT_INITIAL);	  
    /* END SUPPORT */
    ...
    /* RASUPPORT */
    // Method modified. We added myNode, which is the reference of the node that absorbs the target
    targetData.becomeBiomass(target, myNode);    
    /* END RASUPPORT */
  }
  ...
  public void swapHyphae(MycoNode target) {
    ...
    //targetData.become(target, myNode, data.getState());
    
    // TO PROBE
          if((data.getState().equals(HyphaType.EXTENDING) || data.getState().equals(HyphaType.BRANCHING) || data.getState().equals(HyphaType.IMMOBILE)) 
                  && target.isBiomass()) {
              target.getRASupport().cleanIndexTable();
          }
    ...
    // UNCOMMENT
    // Advertisement agent (initial) when a SP (n) transforms into NP                     
        //logMessage("SUPER-PEER " + target.getAlias() + " has received a RS from " + myNode.getAlias()); // TRICK
        myNode.getRASupport().advertiseResourcesTo(target);
        logMessage("HL2");
    /* END RASUPPORT */          
  }
  ...
  public MycoNode growHypha() {
    ...
    if (newHypha != null) {
      ...
      newHyphaData.becomeExtending(newHypha);
        ...
        /* RASUPPORT */
        // UNCOMMENT TRICK
        // If a normal peer (newHypha) of node is going to be promoted to a extending hypha, so we delete newHypha from the database of node        
            //logMessage("SUPER-PEER " + myNode.getAlias() + " DELETES " + newHypha.getAlias() + " from its database"); // TRICK
            myNode.getRASupport().deleteIndexEntry(newHypha.getAlias());        
        /* END RASUPPORT */
    }
    return newHypha;
  }
