...
public class HyphaData implements CDProtocol, Cleanable {
  ...
  public void become(MycoNode n, HyphaType t) {
    ...
    if (n != null) {
        MycoList list = n.getMycoCast().getAllHyphae();
        MycoNode spx = (list.size() > 0) ? n.getMycoCast().getAllHyphae().get(0) : null;
      ...
      //if(neighbor.isBiomass()) {
		  //SupportResourceAggregation.sendAdvertisementAgent(n, target, 0);
	  //}
          // UNCOMMENT**
          logMessage(n + " becomes " + t);
          
          // If a normal peer dies, its super-peer delete it from the database
          if(t.equals(HyphaType.DEAD) && oldState.equals(HyphaType.BIOMASS)) {
                for(MycoNode sp: myNode.getMycoCast().getAllHyphae()) {
                    //System.out.println("SP " + sp.getAlias() + " deletes " + myNode.getAlias());
                    sp.getRASupport().deleteIndexEntry(myNode.getAlias());
                }
          }
          
          // If a super-peer becomes normal-peer, then the super-peer restarts its database
          if((oldState.equals(HyphaType.EXTENDING) || oldState.equals(HyphaType.BRANCHING) || oldState.equals(HyphaType.IMMOBILE)) &&
                  t.equals(HyphaType.BIOMASS)) {
                                          
              // SP (n) restarts its database, beacuse now is a normal peer
              logMessage(n.getAlias() + " has restarted its database, because now is a normal-peer"); 
              n.getRASupport().cleanIndexTable();                                          
          }
  ...
  // When a NP (target) transforms into a SP (origin)
  public void become(MycoNode target, MycoNode origin, HyphaType t) {
    HyphaData d = target.getHyphaData();
    HyphaType oldState = this.state;
    state = t;

    if (target != null) {
		log.log(Level.FINER, target.getID() + " BECOMES " + t + " (WAS "
              + oldState + ")", target);
		fireNodeStateChanged(target,t, oldState);
		MycoCast mycocast = target.getMycoCast();
		mycocast.become(target, t);      
		// Transferring the statistics list and normal peers from origin (SP)) to target (NP)
		//SupportResult.printMessage("SP " + origin.getID() + " transfers its statistic list and normal peers to PN " + target.getID());    
    }
  }
  
  public void becomeBiomass(MycoNode oldSuperpeer, MycoNode newSuperpeer) {
    become(oldSuperpeer, HyphaType.BIOMASS);
    
    /* RASUPPORT */
    // Advertise resources to the peer (newSuperpeer) that absorbs to oldSuperpeer
    //logMessage("SUPER-PEER " + newSuperpeer.getAlias() + " has received a RS from " + oldSuperpeer.getAlias()); // TRICK
    oldSuperpeer.getRASupport().advertiseResourcesTo(newSuperpeer);
    logMessage("HD 1");
    /* END RASUPPORT */
  }
